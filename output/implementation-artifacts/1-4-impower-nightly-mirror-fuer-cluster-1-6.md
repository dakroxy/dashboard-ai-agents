...
- 2026-04-24: Checkbox-Sync fuer Runde-2-Review-Patches — alle 15 Findings sind code-seitig seit Commits 2026-04-22/23 umgesetzt (Model `server_default=text("'[]'")`, phase-Enum in `_reconcile_object`, `cid_raw is None`-Skip, NaN/Inf-Guard in `_normalize_voting_stake` + `round(val, 4)`, Dedup in `_normalize_mandate_refs` via `seen_ids`, Owner-Duplikat-Guard via `seen_cids`, TZ-naive-Guard in `_to_berlin`, `MIRROR_RUN_HOUR/MIRROR_RUN_MINUTE` aus `_sync_common` importiert, Status-Pills in Historie-Tabelle, Skipped-Audit try/except, `test_mirror_e2e_three_objects_one_failure` schaerft phase+counters, `test_mirror_one_property_503_others_succeed` vorhanden, `test_sync_status_anonymous_access_redirects_to_login` + `test_trigger_mirror_run_hx_request_returns_hx_redirect_header` vorhanden). Checkboxen in Review-Findings-Runde-2 auf `[x]` nachgezogen.
